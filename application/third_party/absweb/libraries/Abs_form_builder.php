...
class ABS_Form_Builder {
	...
	public function form_begin($action = '', $attributes = '', $hidden = array()) {
		if(empty($attributes['class'])) {
			$attributes['class'] = 'am-form am-form-horizontal';
		} else {
			$attributes['class'] .= ' am-form am-form-horizontal';
		}
		...
		echo '<div class="am-g">';
		echo '  <div class="am-u-sm-12 am-u-md-12">';
	...
	*/
	...
	public function multi_inline($ids, $name, $types, $default = array(), $class = array(), $param = array(), $prepend = array(), $append = array()) {		
		for ($i = 0; $i < count($types); $i++) {
			$type = isset($types[$i]) ? $types[$i] : '';
			$id = isset($ids[$i]) ? $ids[$i] : 0;
			$df = isset($default[$i]) ? $default[$i] : '';
			...
			switch ($type) {
				case self::$TYPES->TEXT:
					echo "<input type='text' class='input-small $cl' placeholder='$df'/>";
					break;
				case self::$TYPES->OPTION: // TODO
					$this->_build_option($id, $pm, $df, $cl);
					break;
				case self::$TYPES->CHECKBOX: // TODO
					$this->_build_checkboxes($id, $pm, $df, $cl);
					break;
				case self::$TYPES->DATE: // TODO
					$this->_build_date($id, $df, $cl, $pm);
					break;
				case self::$TYPES->BUTTON: // TODO
					$this->single_button($df, $id, $cl, '', ($pp) ? $pp : 'button');
					break;
				case self::$TYPES->PASSWORD:
					echo "<input type='password' class='input-small $cl' placeholder='$df'/>";
					break;
			}
		}
	}
...
//		}
//		echo '<div class="form-group '. $lesser .' '. $error . '">';
		echo "<div class='am-form-group'>";
		echo "	<label class='am-u-sm-3 am-form-label' for='$for'>$name</label>";
		echo "  <div class='am-u-sm-9'>";
	}
	...
	private function _build_text($id, $default, $class, $placeholder, $prepend, $append, $is_password = false) {
		...
		echo '<input type="'. $type .'" '. $readonly .' placeholder="'. $placeholder .'" class="am-u-md-10 am-u-sm-5 '. $class . '" name="'. $id .'" id="'. $id .'" value="'. $value .'" /> ';
	...
	private function _build_button($id, $name, $class, $icon, $type = 'submit') {
		...
		echo "<button type='{$type}' id='{$id}' name='{$id}' {$disabled} class='am-btn {$class} {$disabled}'>{$icon}{$name}</button>";
	}
	...
	*/
	private function addon_begin($prepend, $append, $class) {
		if ($append != '' || $prepend != '') {
			...
			if ($prepend != '') {
				echo '	<span class="input-group-addon">'. $prepend .'</span>';
			}
		}
	}
